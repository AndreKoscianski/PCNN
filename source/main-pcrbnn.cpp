...
//   return DerivadaParabolaTeste (v.todouble())
//           +
...
void GerarMatrizSaida () {
   ...
   //unlink ("output2.csv"); // paranoia - fopen já faz isso.

   FILE *arq  = fopen ("output2.csv", "wt");
   //FILE *arq1 = fopen ("deriv1.csv", "wt");
   //FILE *arq2 = fopen ("deriv2.csv", "wt");
   ...
   for (it = 0; it < 16; it++) {
      ...
      for (ix = 0; ix < 15; ix++) {
         ...
         //fprintf (arq1, "%f,", Rede.deriv (1, 0, 0));
         //fprintf (arq2, "%f,", Rede.deriv (2, 0, 0));
      }
